...
- **Render pipeline**: `filterMovies()` filters `allMovies` (text query across title/ai_title/file_name/extracted_name/genre/actors/director/plot/user_tags, plus genre/decade/language/min-IMDb `activeFilters`), then a **sort stage** `sortMovies()` applies the active mode before rendering. Sort modes (segmented control, persisted via `savePref('sort', …)`, applied by `setSort`): `recent` (baseline `_order`), `yearDesc`/`yearAsc`, `ratingDesc`, `titleAsc`.
- **v1.5.0 additions**: a collapsible **Library-stats panel** derived from `allMovies` (totals, average IMDb rating, your-rated count, clickable top-genre chips; dismiss/collapse persisted); a **min-IMDb quick filter** (Any/7+/8+) wired into the filter chips + "Showing X of Y" count; a **density toggle** (Comfortable/Compact, persisted); **keyboard grid navigation** (arrow keys move a focus ring, Enter opens Info); a **scroll-to-top** button; and a **richer detail modal** that parses `additional_info` into score chips, renders genre/actor chips, and shows a `safePosterUrl`-validated blurred poster backdrop. All new dynamic values are `escapeHtml`/`textContent`-safe.
...
- A **segmented `role="tablist"`** with three tabs — **Pending / Enriched / Failed** — carrying **live counts**. Classification: *enriched* = has a real OMDb `title`; *failed* = `is_active == 4` (the failed tab surfaces movies that failed OMDb so they can be fixed; failed rows' primary button reads **Retry**); *pending* = the rest. `switchTab()` toggles `aria-selected` and re-renders.
- A **client-side search + sort** over the current tab's rows (search across `file_name`/`title`/`ai_title`/`extracted_name`/`user_tags`; sort persisted via `savePref`).
- **Inline validation** on the per-row inputs (IMDb `^tt\d+$`, 4-digit year, 0–10 rating) with an invalid state; **Save enables only when a valid change is made** (dirty-check vs `data-orig`); **Enter** in a row's input triggers its save.
- **Per-row quick actions** ▶ Play (`POST /play {uuid}`) and 📂 Open folder (`POST /api/open-folder {uuid}`) via a shared `postJson` helper, with the same loading/disabled + toast pattern as the viewer.
- **Skeleton / empty / error states**; a **premium info modal** (`#infoModal`, `role="dialog"` + `aria-modal`, **Tab focus-trap** + Esc + focus restore) with Play / Open-folder / safe Open-on-IMDb actions. All movie fields are `escapeHtml`/`safePosterUrl`-escaped before insertion.
